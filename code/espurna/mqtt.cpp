...
void mqttSend4Scale(const char * topic, const char * message) {

    // Enqueue new message
    mqttEnqueue(topic, message);

    // Reset flush timer
    _mqtt_flush_ticker.once_ms(MQTT_USE_JSON_DELAY, mqttFlush4Scale);
}
...
void mqttFlush4Scale() {
    ...
    root[MQTT_TOPIC_TURN] = g_mqtt_packet_data.number_n;
    ...
    else
    {
        ...
        #endif
        #ifdef LORA_RECEIVER
            // relayStatus(DATA_SIGNAL_RECEIVER, true);
        #endif
