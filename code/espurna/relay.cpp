...
uint8_t _relayAdd(void) {
    static int output_id = _relays.size();
    // relay_t qrelay(GPIO_NONE, RELAY_TYPE_NORMAL, GPIO_NONE);
    // _relays.push_back(qrelay);
    _relays.emplace_back();

    uint8_t id = (uint8_t)output_id;
    output_id++;

    DEBUG_MSG_P(PSTR("[RELAY] Add relay id: %d\n"), id);
    return id;
}
